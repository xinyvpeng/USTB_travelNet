...
照片数量: ${n}`)}async onEnter(){console.log("进入旅游记录视图"),await this.updateRecordsList()}async onLeave(){console.log("离开旅游记录视图"),this.hideEditModal()}}N(x,"MAX_PHOTOS_PER_RECORD",10);class z extends S{constructor(t,e={}){super(t,e),this.storageInfo=null}async render(){this.showLoading("正在加载数据管理...");try{if(!window.AppState||!window.DataManager)throw new Error("应用未完全初始化");const t=this.getTemplate();this.updateContent(t),await this.loadStorageInfo(),this.bindEvents(),this.isRendered=!0}catch(t){this.showError("加载数据管理视图失败",t)}}getTemplate(){return`
      <div class="data-view">
        <div class="view-header">
